...
// src/index.ts
import { Cache as Cache2, ColorManagement } from "three";
  ...
  if (adapter) {
    for (const [, p] of [["_", adapter], ...Object.entries(adapter)]) {
      if (p.prototype instanceof AbstractAdapter) {
        checkVersion(p.id, p.VERSION, "5.5.1");
        return p;
      }
    }
  }
...
import { MathUtils as MathUtils3, Mesh as Mesh2, SphereGeometry } from "three";
...
var EquirectangularAdapter = class extends AbstractAdapter {
  ...
  async loadTexture(panorama, newPanoData, loader = true, useXmpPanoData = this.config.useXmpData) {
...
EquirectangularAdapter.VERSION = "5.5.1";
  ...
  setOverlay(path, opacity = this.config.overlayOpacity) {
    ...
    if (!path) {
      ...
      return this.adapter.loadTexture(
        path,
        (image) => {
          if (this.state.textureData.panoData.isEquirectangular) {
            const p = this.state.textureData.panoData;
